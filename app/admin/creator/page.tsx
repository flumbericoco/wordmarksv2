...
export default function CreatorPage() {
  ...
  return <div className="admin-page admin-enter space-y-6">
    ...
    <section className="rounded-[28px] border border-black/10 bg-white/70 p-6 sm:p-8"><div className="mb-5 flex items-center justify-between"><div><h2 className="text-xl font-black">Instructions</h2><p className="text-xs text-black/45">Private system prompt prepended to every request. Safety and negative-quality rules are enforced automatically.</p></div><span className="rounded-full bg-[#d9d3ff] px-3 py-1 text-[10px] font-black">PRIVATE</span></div><textarea value={settings.systemPrompt} onChange={(e) => setSettings({ ...settings, systemPrompt: e.target.value })} maxLength={20000} rows={18} className="w-full resize-y rounded-2xl border border-black/10 bg-[#f5f3ed] p-5 font-mono text-sm leading-6 outline-none focus:border-[#6447e8]" placeholder="You are a world-class identity designer…"/><p className="mt-2 text-right text-[10px] text-black/35">{settings.systemPrompt.length.toLocaleString()} / 20,000</p></section>
    <section className="rounded-[28px] border border-black/10 bg-white/70 p-6 sm:p-8">
      <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-black">Knowledge</h2><p className="text-xs text-black/45">Visual references are inspected directly; text documents are injected into the generation context.</p></div><label className="flex items-center gap-2 text-xs font-bold"><input type="checkbox" checked={settings.knowledgeBaseEnabled} onChange={(e) => setSettings({ ...settings, knowledgeBaseEnabled: e.target.checked })}/> Use knowledge</label></div>
      <button onClick={() => input.current?.click()} disabled={uploading} className="mt-6 grid min-h-28 w-full place-items-center rounded-2xl border-2 border-dashed border-black/15 bg-[#f5f3ed] text-sm font-bold transition hover:border-[#6447e8] disabled:opacity-50">{uploading ? 'Uploading…' : '+ Upload PNG, JPG, WebP, TXT, MD, CSV, or JSON'}</button>
      <input ref={input} className="hidden" type="file" multiple accept="image/png,image/jpeg,image/webp,.txt,.md,.markdown,.csv,.json" onChange={(e) => void upload(e.target.files)}/>
      <div className="mt-5 grid gap-3 sm:grid-cols-2">{items.map((item) => <div key={item.id} className="flex items-center gap-3 rounded-2xl border border-black/10 bg-[#f5f3ed] p-3">
        {item.kind === 'image' ? <img src={`/api/v1/admin/knowledge-base?id=${encodeURIComponent(item.id)}&image=1`} alt={`Reference ${item.filename}`} className="h-16 w-20 rounded-lg bg-white object-contain"/> : <div className="grid h-16 w-20 place-items-center rounded-lg bg-white text-[10px] font-black">DOC</div>}
        <div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{item.filename}</p><p className="text-[11px] text-black/40">{item.kind === 'image' ? 'Visual reference' : 'Knowledge document'} · {item.category}</p>{item.description ? <p className="mt-1 line-clamp-2 text-[11px] text-black/50">{item.description}</p> : null}</div>
        <button onClick={() => void remove(item)} className="text-xs font-bold text-red-600">Remove</button>
      </div>)}{!items.length && <p className="py-8 text-center text-sm text-black/35">No knowledge files yet.</p>}</div>
    </section>
    <section className="rounded-[28px] border border-black/10 bg-white/70 p-6 sm:p-8"><p className="text-xs font-black uppercase tracking-wider">Production output</p><p className="mt-2 text-sm text-black/55">OpenAI image generation runs at maximum quality on a landscape production canvas. Users receive PNG first and may create an SVG export afterward.</p></section>
  </div>;
}
