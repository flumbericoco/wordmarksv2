...
export default function AdminBillingPage() {
  ...
  return <div className="space-y-8">
    ...
    <section className="overflow-x-auto rounded-3xl border border-black/10 bg-white/60 p-6"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-black">Recent payment transactions</h2><p className="mt-1 text-xs text-black/45">Stripe payments only. Manual credit changes are recorded in the audit and credit ledgers.</p></div><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search user, kind, status" className="rounded-xl border bg-white px-4 py-2 text-sm"/></div><table className="mt-4 w-full text-left text-sm"><thead><tr><th>User</th><th>Kind</th><th>Amount</th><th>Credits</th><th>Status</th></tr></thead><tbody>{matchingTransactions.map((row) => <tr key={String(row.id)} className="border-t"><td className="py-3">{row.email}</td><td>{row.kind}</td><td>{String(row.currency).toUpperCase()} {(Number(row.amount) / 100).toFixed(2)}</td><td>{row.credits}</td><td>{row.status}</td></tr>)}</tbody></table>{matchingTransactions.length === 0 ? <p className="py-5 text-sm text-black/45">No payment transactions yet.</p> : null}</section>
