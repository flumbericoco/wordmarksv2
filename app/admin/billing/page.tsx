...
export default function AdminBillingPage() {
  ...
  return <div className="space-y-8">
    ...
    <section className="overflow-x-auto rounded-3xl border border-black/10 bg-white/60 p-6"><h2 className="text-xl font-black">Recent transactions</h2><table className="mt-4 w-full text-left text-sm"><thead><tr><th>User</th><th>Kind</th><th>Amount</th><th>Credits</th><th>Status</th></tr></thead><tbody>{data.transactions.map((row) => <tr key={String(row.id)} className="border-t"><td className="py-3">{row.email}</td><td>{row.kind}</td><td>{String(row.currency).toUpperCase()} {(Number(row.amount) / 100).toFixed(2)}</td><td>{row.credits}</td><td>{row.status}</td></tr>)}</tbody></table></section>
