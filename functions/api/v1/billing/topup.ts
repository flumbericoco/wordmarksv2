...
export const onRequestPost: PagesFunction<Env> = async ({request,env}) => {
  ...
  const form=new URLSearchParams({mode:'payment',success_url:`${origin}/account?checkout=success&session_id={CHECKOUT_SESSION_ID}`,cancel_url:`${origin}/account?checkout=cancelled`,'line_items[0][price]':env.STRIPE_PRICE_INITIAL_25,'line_items[0][quantity]':'1','metadata[user_id]':user.id,'metadata[kind]':'topup'});
